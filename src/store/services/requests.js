...
import { parseURL, parseConfig, parseBaseURL } from '_utils/request'
import { API_URL } from '_config/environment'
import { createFormData } from '_utils/helpers'

import { byPassToken } from '../../constants/constant'
...
const parseParams = (url, config, data, baseURL = null) => method => {
  ...
  const axiosConfigs = {
    baseURL: parseBaseURL(baseURL, emptyBaseURL), // Dynamically update the base url if needed
    method,
  ...
  if (configParams.queryParams) {
    /* eslint-disable prefer-template */
    const queryString = Object.keys(configParams.queryParams)
      .map(key => key + '=' + configParams.queryParams[key])
      .join('&')
    axiosConfigs.url += '?' + queryString
  }
